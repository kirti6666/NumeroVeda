...
async function deliver(order:Order,kind:'payment'|'delivery'){
 ...
 try{const transport=nodemailer.createTransport({host:process.env.SMTP_HOST,port:Number(process.env.SMTP_PORT||587),secure:process.env.SMTP_PORT==='465',connectionTimeout:10000,socketTimeout:15000,auth:process.env.SMTP_USER?{user:process.env.SMTP_USER,pass:process.env.SMTP_PASS}:undefined});await transport.sendMail({from:process.env.MAIL_FROM,to:order.email,bcc:process.env.ADMIN_EMAIL||undefined,...await mailContent(order,kind),messageId:kind==='payment'?`<payment-${order.id}@numeroveda.local>`:undefined});await record('sent');return true;}catch{await record('failed','Email delivery failed. Check SMTP configuration.');return false;}
}
