...
export const orderSchema=z.object({productId:text(100).min(1),name:text(120).min(2),email:z.email().max(200),phone:z.string().regex(/^[6-9]\d{9}$/,'Enter a 10-digit Indian mobile number.'),dob:z.string().default(''),question:text(3000).default(''),answers:z.record(z.string().max(60),text(3000)).default({}),language,consent:z.literal(true)}).superRefine((input,ctx)=>{
